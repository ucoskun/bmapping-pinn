...
class PINN(tf.keras.Model):
    ...
    def train_step(self, o1):
        ...
        with tf.GradientTape(persistent = True) as g:
            ...
            #loss_cross = tf.reduce_mean(huber(u_zy, u_yz) + huber(u_xz, u_zx) + huber(u_yx, u_xy))
            loss_u = tf.reduce_mean(tf.square(u_pred - self.u_labels))
            #loss_u = tf.reduce_mean(tf.reduce_sum(tf.square(u_pred - self.u_labels), 1))
            loss_f = tf.reduce_mean(tf.square(f))
            
            
            #loss = loss_f + 100*loss_u + loss_cross
        ...
        #loss_val = tf.reduce_mean(tf.square(self.validation_labels-self(self.validation_data)))
...
if __name__ == "__main__":
    ...
    #x_u = np.concatenate((-L*np.ones([N_u, 1]), L*np.ones([N_u, 1]), 
    #                      np.repeat(np.random.default_rng().uniform(low = -L, high = L, size = (N_u, 1)), 4, axis = 0)),
    #                      axis = 0)
    #y_u = np.concatenate((np.repeat(np.random.default_rng().uniform(low = -L, high = L, size = (N_u, 1)), 2, axis = 0), 
    #                      -L*np.ones([N_u, 1]), L*np.ones([N_u, 1]), 
    #                      np.repeat(np.random.default_rng().uniform(low = -L, high = L, size = (N_u, 1)), 2, axis = 0)),
    #                      axis = 0)
    #z_u = np.concatenate((np.repeat(np.random.default_rng().uniform(low = -L, high = L, size = (N_u, 1)), 4, axis = 0), 
    #                      -L*np.ones([N_u, 1]), L*np.ones([N_u, 1])),
    #                      axis = 0)
    ...
    #print(np.mean(u_labels, axis=0))
    #print(np.std(u_labels, axis=0))


    # Points on the cube
    #fig = plt.figure()
    #ax = fig.add_subplot(projection='3d')
    #ax.scatter(x_u, y_u, z_u)
    #plt.show()
    ...
    #layers = [3, 128, 128, 128, 128, 128, 3]
    ...
    #reduce_lr = tf.keras.callbacks.ReduceLROnPlateau(monitor='Loss', factor=0.2, min_delta=0.00000001,
    #                          patience=300, min_lr=0.000000001, cooldown=200)
    ...
    #matlab_dic = {"y_real": temp_final, "y_pred": model_output.numpy()}
    #savemat("B.mat", matlab_dic)
    ...
    #model_output = tf.reshape(model_output, [N_val, 1])
    #temp_final = tf.cast(temp_final, dtype=tf.float32)
    #temp_final = np.sqrt(np.sum(np.square(temp_final), axis=1))
    #temp_final = tf.sqrt(tf.reduce_sum(tf.square(model.call(inputs)-temp_final), axis=1))
    #print(temp_final.shape)
    #print(model_output.shape)
    ax.plot3D(x_test, y_test, final_plot)
    #ax.scatter3D(x_test_np, y_test_np, temp_final)
    plt.show()
    #print(temp_final.shape)
    #temp_final = temp_final.reshape((N_val, 3))
    #temp_final = temp_final[:,2]
    #plt.plot(z_test, temp_final)
    #plt.show()

    #inputs = tf.concat([x_test, y_test, z_test], axis = 1)
    #plt.plot(z_test, model.call(inputs)[:,2])
    #temp_final = np.array([B(x_test[i], y_test[i], z_test[i]) for i in range(N_val)])
    #print(temp_final.shape)
    #temp_final = temp_final.reshape((N_val, 3))
    #temp_final = temp_final[:,2]
    #plt.plot(z_test, temp_final)
